...
class ExtensionContextContentSecurityProlicyArbiter implements ContentSecurityPolicyArbiter {
	constructor(
		private globalState: vscode.Memento
	) { }

	public isEnhancedSecurityDisableForWorkspace(): boolean {
		return this.globalState.get<boolean>('trusted_preview_workspace:' + vscode.workspace.rootPath, false);
	}

	public addTrustedWorkspace(rootPath: string): Thenable<void> {
		return this.globalState.update('trusted_preview_workspace:' + rootPath, true);
	}

	public removeTrustedWorkspace(rootPath: string): Thenable<void> {
		return this.globalState.update('trusted_preview_workspace:' + rootPath, false);
	}
...
export function activate(context: vscode.ExtensionContext) {
	...
	const cspArbiter = new ExtensionContextContentSecurityProlicyArbiter(context.globalState);
	...
	context.subscriptions.push(vscode.commands.registerCommand('markdown.showPreviewSecuritySelector', (resource: string | undefined) => {
		...
		vscode.window.showQuickPick<PreviewSecurityPickItem>(
			[
				...
				{
					id: PreviewSecuritySelection.Close,
					label: localize('preview.showPreviewSecuritySelector.close', 'Close'),
					description: ''
				},
			], {
				...
				switch (selection && selection.id) {
					...
					case PreviewSecuritySelection.Close:
						return false;
				}
				return false;
			}).then(shouldUpdate => {
